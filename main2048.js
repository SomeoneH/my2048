	...
	// 手机划动的时候，屏幕会跟着一起动。判断touchmove的位置，如果是在操作区域内，则取消划动
	gridContainer.addEventListener('touchmove', function(event) {
		// 计算操作局域的左右边界
		// var leftStart = gridContainer.offsetLeft;
		// var rightEnd = gridContainer.offsetLeft + gridContainer.offsetWidth;
		// var topStart = gridContainer.offsetTop;
		// var bottomEnd = gridContainer.offsetTop + gridContainer.offsetTop;
		// if (event.targetTouches[0].pageX >= leftStart &&
		// event.targetTouches[0].pageX <= rightEnd &&
		// event.targetTouches[0].pageY >= topStart &&
		// event.targetTouches[0].pageY <= bottomEnd) {
		// 	event.preventDefault();
		// }
		event.preventDefault();
	})
